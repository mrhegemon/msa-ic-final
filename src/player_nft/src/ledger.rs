...
impl Ledger {
    ...
    pub async fn updateTokenMetadata(&mut self, from: Principal, token_identifier: &TokenIdentifier, metadata_desc: &MetadataDesc) -> Result<bool, String> {
        ...
        if token_metadata.principal != from {
            return Err("Unauthorized, token not owned by caller".to_string());
        } else {
            token_metadata.metadata_desc = metadata_desc.clone();
            return Ok(true)
        }
    }
    
    pub async fn updatePlayersAge(&mut self, token_ids: &TokenIds) -> Result<bool, String> {
        
        for token_identifier in token_ids {
            let token_index = into_token_index(&token_identifier.to_string());
            let token_metadata = ledger()
                .tokens
                .get_mut(&token_index)
                .expect("unable to find token identifier in tokens");

            if let Some(item) = (&mut token_metadata.metadata_desc[0].key_val_data).iter_mut().find(|item| item.key == "age") {
                if let MetadataVal::Nat8Content(value) = item.val {
                    item.val = MetadataVal::Nat8Content(value + 1); // Increase existing player's age by one
                }
            }
        }
    ...
    pub fn get_sale_token_ids_for_user(&self, user: &Principal) -> Vec<Listing> {
        ledger()
            .listed
            .iter()
            .filter(|x| x.owner == *user)
            .cloned()
            .collect()
    }

    pub fn get_buy_token_ids_for_user(&self, user: &Principal) -> Vec<Listing> {
        ledger()
            .listed
            .iter()
            .filter(|x| x.owner != *user)
    ...
    pub async fn list(&mut self, from: Principal, token_identifier: &TokenIdentifier, price: Nat) -> Result<bool, String> {
        ...
        let token_wrapped: Option<&TokenMetadata> = ledger()
            .tokens
            .get(&token_index);

        if token_wrapped.is_some() {
            let token: &TokenMetadata = token_wrapped.unwrap();
