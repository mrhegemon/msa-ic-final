...
impl Default for StatsData {
    fn default() -> Self {
        StatsData {
            ...
            royalty: 0_f64,
            royalty_to: Principal::anonymous(),
            history_size: 0,
            deploy_time: 0,
            nft: Principal::anonymous(),
        }
    }
}
...
fn _calc_royalty_fee(value: Nat) -> Nat {
    let stats = ic::get_mut::<StatsData>();
    Nat::from(((stats.royalty * value.clone().0.to_f64().unwrap()) / 100_f64).round() as u64)
}

/*fn _calc_charge_fee(value: Nat) -> Nat {
    let stats = ic::get_mut::<StatsData>();
    Nat::from(((stats.fee * value.clone().0.to_f64().unwrap()) / 100_f64).round() as u64)
}*/
...
#[update(name = "transferFromWithoutApprove")]
#[candid_method(update, rename = "transferFromWithoutApprove")]
async fn transfer_from_without_approve(from: Principal, to: Principal, value: Nat) -> TxReceipt {
    let owner = ic::caller();
    let stats = ic::get_mut::<StatsData>();

    // Let's check method caller (owner) is NFT canister or not,    
    if owner != stats.nft {
        // If method caller is not NFT canister then return Unauthorized error
    ...
    let royalty_fee = _calc_royalty_fee(value.clone());

    // Royalty
    _transfer(from, stats.royalty_to, royalty_fee.clone());
    stats.history_size += 1;
    ...
    // Owner
    _transfer(from, to, value.clone() - royalty_fee.clone());
    stats.history_size += 1;
